...
const WINDOWS_SHIP_DB_DIR_BACKSLASH: &str = r"\\Mserver\USA_DB\test_jn\ship_db";
...
fn spawn_backend(app: &tauri::AppHandle, port: u16) -> Result<(), String> {
    ...
    #[cfg(windows)]
    let command = command
        .env("SHIP_DB_DIR", WINDOWS_SHIP_DB_DIR)
        .env("SHIP_DB_DIR_BACKSLASH", WINDOWS_SHIP_DB_DIR_BACKSLASH);
...
mod tests {
    ...
    fn windows_sidecar_db_dir_uses_forward_slash_unc_path() {
        ...
        assert_eq!(
            WINDOWS_SHIP_DB_DIR_BACKSLASH,
            r"\\Mserver\USA_DB\test_jn\ship_db"
        );
    }
